...
def _preprocess_multi_modal_inputs(prompt_str, processor, **kwargs):
    ...
    # 2. 遍历并校验每一张图像（包括 zoom_in 后的切片）
    for img in input_mm_data["image"]:
        # --- A. 零尺寸校验：防止 0x0 像素崩溃 ---
        if img.width <= 0 or img.height <= 0:
            print(f"🚨 [DEBUG] Detected empty image ({img.width}x{img.height}). Using black placeholder.")
            img = Image.new('RGB', (224, 224), (0, 0, 0))

        # --- B. 长宽比校验：防止 Qwen smart_resize 报错 (AR > 200) ---
        # 计算绝对长宽比：$$AR = \max(\frac{W}{H}, \frac{H}{W})$$
        w, h = img.size
        aspect_ratio = max(w / h, h / w)
        
        # Qwen 官方阈值是 200，我们设为 195 留出安全余量
        if aspect_ratio > 195:
            print(f"🚨 [DEBUG] Aspect ratio too extreme ({aspect_ratio:.2f}). Resizing to 1:1.")
            # 强制调整为 224x224，消除畸形长宽比，确保训练不中断
            img = img.resize((224, 224), Image.Resampling.LANCZOS)

        # 3. 将 PIL Image 序列化为字节流
        buf = io.BytesIO()
        try:
            img.save(buf, format='PNG')
            png_bytes = buf.getvalue()
            # 构造 verl/qwen_vl_utils 期待的格式
            image_info_list.append({"bytes": png_bytes})
        finally:
            buf.close()

    # 4. 调用底层 vision_utils 进行特征提取准备
    # 注意：这里需要确保 process_image 能正确处理 bytes 格式
    input_mm_data["image"] = [process_image(img_info) for img_info in image_info_list]
    
    # 5. 调用 Processor 生成多模态张量
    model_inputs = processor(
        text=[vllm_input_prompt], 
        images=input_mm_data["image"], 
        return_tensors="pt"
    )
    
    # 6. 提取必要的 Tensor 并处理冗余键
    input_ids = model_inputs.pop("input_ids")[0]
    # attention_mask 虽然没直接返回给 vLLM，但保留在 mm_inputs 中
    _ = model_inputs.pop("attention_mask")[0] 

    # 移除训练/推理中不需要的元数据键，防止后续 forward 报错
