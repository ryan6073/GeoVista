...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        """
        import json
        # 1. 从 dataframe 中获取原始数据，并显式转为 dict 以确保可修改性
        row_dict: dict = dict(self.dataframe[item])

        # --- [关键修改：反序列化 JSON 字符串字段] ---
        # 针对 AgentZoom 数据集中的 extra_info 和 reward_model 进行处理
        for key in ["extra_info", "reward_model"]:
            ...
            if isinstance(val, str):
                try:
                    ...
                    # 如果数据损坏或非 JSON 格式，设为空字典防止下游 crash
                    row_dict[key] = {}
        # -----------------------------------------
